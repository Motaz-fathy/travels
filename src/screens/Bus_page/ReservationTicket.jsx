...
export const ReservationTicket = () => {
  ...
  const { reservationReturnTicket } = useSelector(
    state => state.CreateReturnTicketReducer
  );
  const { tripType } = useSelector(state => state.tripReducer);
  const { paymentData, loading, error } = useSelector(
 ...
 */
    ...
    if (data.length === 0)
      return (
        <div className="w-full min-h-screen flex justify-center items-center text-4xl">
          not reservation ticket data here{" "}
        </div>
      );
    return (
      <div className="w-full flex flex-col items-center">
        {data.trips.map((trip , index ) => {
          return <div key={index} className="w-full flex flex-col items-center gap-6 ">
            <div className="flex flex-col items-center gap-4 bg-gray-50 h-auto shadow-2xl mx-auto  reservation-ticket rounded-md w-5/6 max-md:w-full px-6 py-10">
              <div className="w-full flex justify-between items-center  ">
                ...
                <span className="flex justify-center items-center gap-2">
                  <FaRegClock />{" "}
                  <span>{getTimeString(trip.date_time)}</span>
                </span>
              </div>
        ...
        <div
          onClick={() => handlePayment(data.id, token)}
  ...
  //           <div className="flex flex-col items-center gap-4 bg-gray-50 h-auto shadow-2xl mx-auto  rounded-md w-5/6 max-md:w-full px-6 py-10">
  //             <div className="w-full flex justify-between items-center  ">
  //               <span className="px-3 bg-gray-800 rounded-full py-1 text-gray-200 ">
  //                 oneWay
  //               </span>
  ...
  return (
    <div className="w-full flex flex-col items-center bg-gray-200">
      ...
      <div className="h-auto w-full flex justify-center items-center gap-4 my-12 px-10 max-md:px-4">
        ...
        {tripType === "round" && RenderTicket(reservationReturnTicket)}
      </div>
      {/* footer  */}
