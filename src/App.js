...
function App() {
  ...
  const routes = [
    ...
    { path: "/busTrips/:BusId/reservation-ticket", element: ReservationTicket, protected: true },
      ...
      <Routes>
        {routes.map(({ path, element: Component, protected: isProtected }) => (
          <Route
            key={path}
            path={path}
            element={isProtected ? <ProtectedRoute element={Component} /> : <PublicRoute element={Component} />}
          />
        ))}
      </Routes>
    </Router>
